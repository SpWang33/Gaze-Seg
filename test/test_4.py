...
heatmap_dir = os.path.join(data_dir, 'feature_fixationheatmap_5')
mask_dir = os.path.join(data_dir, 'frame_groundtruth')

# List the files in each of the directories
for frame, fixation, heatmap, mask in zip(
        sorted(os.listdir(frame_dir)), sorted(os.listdir(fixation_dir)),
        sorted(os.listdir(heatmap_dir)), sorted(os.listdir(mask_dir))):
...
for idx, d in enumerate(test_dataloader):
    ...
    with torch.no_grad():
        ...
        sparse_embeddings, dense_embeddings = predictor.model.sam_prompt_encoder(points=(unnorm_coords[:, 0], labels),boxes=None,masks=heatmap,)
    ...
    # cv2.imwrite(os.path.join(test_path, 'prd_mask_{}.jpg'.format(idx)), (prd_mask_vis*255).astype(np.uint8)) 
...
np.savez('results/test4.npz', iou=np.array(iou_list), acc=np.array(acc_list))
