...
class gaze_dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        convex_hull = np.array(convex_hull)/255.
        convex_hull = cv2.resize(convex_hull, (256, 256), interpolation=cv2.INTER_NEAREST) 
        convex_hull = np.expand_dims(convex_hull, axis=0)

        return {'image': image, 'ann_map': ann_map, 'new_fixation': new_fixation, 'convex_hull': convex_hull, 'name': d['file']}
...
# load weight
trained_weight = "/mnt/e/AI_project/gaze_rebuttle/training_info/exp_4/2025-01-23_09_31_22/best_weight.torch"
predictor.model.load_state_dict(torch.load(trained_weight))

data_dir = '/mnt/e/AI_project/gaze_rebuttle/6_dataset_full/6_dataset/benchmark/object/4/testing'
...
for frame, fixation, convex_hull, mask in zip(
        sorted(os.listdir(frame_dir)), sorted(os.listdir(fixation_dir)),
        sorted(os.listdir(convex_hull_dir)), sorted(os.listdir(mask_dir))):
...
# test_path = os.path.join("/cluster/scratch/yizchen/gaze_data/training_info/benchmark/scene/test/exp_4")
...
for idx, d in enumerate(test_dataloader):
    ...
    with torch.no_grad():
        ...
        sparse_embeddings, dense_embeddings = predictor.model.sam_prompt_encoder(points=(unnorm_coords[:, 0], labels),boxes=None,masks=convex_hull,)
    ...
    # plt.axis('off')
    # plt.subplot(2, 2, 2)
    # plt.imshow(convex_hull[0][0].detach().cpu().numpy()*255)
    # plt.axis('off')
    # plt.subplot(2, 2, 3)
...
np.savez('results/test3.npz', iou=np.array(iou_list), acc=np.array(acc_list))
