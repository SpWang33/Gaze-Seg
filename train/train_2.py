...
class gaze_dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        fixation_mask = d['fixation_mask']
        fixation_mask = Image.open(fixation_mask)
        fixation_point = np.where(np.array(fixation_mask)==255)
        x_coords, y_coords = fixation_point[1], fixation_point[0]
        # Halve each coordinate and ensure they are within the 256x256 bounds
        x_coords_half = np.clip(x_coords // 2, 0, 255)
        y_coords_half = np.clip(y_coords // 2, 0, 255)
        # Create a blank 256x256 image
        fixation_image = np.zeros((256, 256))
        # Assign a value to the pixels at the halved coordinates
        fixation_image[y_coords_half, x_coords_half] = 255
        fixation_image = np.expand_dims(fixation_image, axis=0)

        return {'image': image, 'ann_map': ann_map, 'new_fixation': new_fixation, 'fixation_mask': fixation_image}
...
fixation_mask_dir = os.path.join(data_dir, 'feature_fixations_5')
mask_dir = os.path.join(data_dir, 'frame_groundtruth')

# List the files in each of the directories
for frame, fixation, fixation_mask, mask in zip(
        sorted(os.listdir(frame_dir)), sorted(os.listdir(fixation_dir)),
        sorted(os.listdir(fixation_mask_dir)), sorted(os.listdir(mask_dir))):
    ...
    data_sample.append(
        {
            ...
            'fixation_mask': os.path.join(fixation_mask_dir, fixation_mask),
...
res_dir = os.path.join(res_dir, 'exp_2', str(datetime.datetime.now()).replace(' ', '_').split('.')[0].replace(':', '_'))
...
for epoch in range(epochs):
    ...
    with tqdm(total=int(n_train), desc=f'Epoch {epoch + 1}/{epochs}', unit='img', bar_format='{desc:<5.5}{percentage:3.0f}%|{bar:10}{r_bar}') as pbar:
        for idx, d in enumerate(train_dataloader):
            ...
            fixation_mask = d['fixation_mask'].cuda().float()
            ...
            mask_input, unnorm_coords, labels, unnorm_box = predictor._prep_prompts(new_fixation, mask_label, box=None, mask_logits=fixation_mask, normalize_coords=True)
            sparse_embeddings, dense_embeddings = predictor.model.sam_prompt_encoder(points=(unnorm_coords[:, 0], labels),boxes=None,masks=mask_input,)
