...
class gaze_dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'image': image, 'ann_map': ann_map, 'new_fixation': new_fixation, 'convex_hull': convex_hull}
...
res_dir = os.path.join(res_dir, 'exp_3', str(datetime.datetime.now()).replace(' ', '_').split('.')[0].replace(':', '_'))
